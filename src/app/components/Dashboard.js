...
import { MONTHLY_DATA, TOP_VIDEOS, LAST_UPDATED } from '../data';
...
// ── Main ──────────────────────────────────────────────────────────────────────
const MONTHS_WITH_DATA = MONTHLY_DATA.filter(m => m.gmv > 0 || m.videos > 0);
...
export default function Dashboard() {
  ...
  // Build quarters
  const { quarters, quarterTabs, monthTabs, chartMonths } = useMemo(() => {
    const qMap = {};
    MONTHS_WITH_DATA.forEach(m => {
      const q = getYearQuarter(m.month);
      if (!qMap[q]) qMap[q] = [];
      qMap[q].push(m);
    });
    const quarters = Object.entries(qMap).map(([key, months]) => ({
      key, label: key.replace('-',' '), months,
    ...
    const monthTabs = MONTHS_WITH_DATA.map(m => {
      const [y,mo] = m.month.split('-');
      return { key:m.month, label:`${MONTH_NAMES[mo]} '${y.slice(2)}`, type:'month', data:m };
    });
    const chartMonths = MONTHS_WITH_DATA.filter(m=>m.gmv>100).map(m => {
      const [,mo] = m.month.split('-');
      return { label:MONTH_NAMES[mo], gmv:m.gmv, views:m.views, videos:m.videos };
    });
    return { quarters, quarterTabs, monthTabs, chartMonths };
  }, []);

  const allTabs = [{ key:'all', label:'All Time', type:'all' }, ...quarterTabs, ...monthTabs];
  const tab = allTabs.find(t => t.key === activeTab) || allTabs[0];

  const agg = useMemo(() => {
    if (tab.type === 'month' || tab.type === 'quarter') return tab.data;
    return {
      gmv:      MONTHS_WITH_DATA.reduce((s,m)=>s+m.gmv,0),
      sold:     MONTHS_WITH_DATA.reduce((s,m)=>s+m.sold,0),
      views:    MONTHS_WITH_DATA.reduce((s,m)=>s+m.views,0),
      videos:   MONTHS_WITH_DATA.reduce((s,m)=>s+m.videos,0),
      creators: Math.max(...MONTHS_WITH_DATA.map(m=>m.creators)),
      samples:  MONTHS_WITH_DATA.reduce((s,m)=>s+m.samples,0),
      messages: MONTHS_WITH_DATA.reduce((s,m)=>s+m.messages,0),
    };
  }, [tab]);
  ...
  const gmvSpark      = MONTHS_WITH_DATA.map(m=>m.gmv);
  const viewsSpark    = MONTHS_WITH_DATA.map(m=>m.views);
  const videosSpark   = MONTHS_WITH_DATA.map(m=>m.videos);
  const creatorsSpark = MONTHS_WITH_DATA.map(m=>m.creators);
      ...
      <div style={{ position:'relative', zIndex:1 }}>
        ...
        <header style={{ background:'rgba(10,11,13,0.95)', backdropFilter:'blur(16px)', borderBottom:'1px solid #1e2129', padding:'12px 24px', display:'flex', alignItems:'center', justifyContent:'space-between', position:'sticky', top:0, zIndex:50 }}>
          ...
          <div style={{ display:'flex', gap:10, alignItems:'center' }}>
            <span style={{ fontFamily:"'DM Mono',monospace", fontSize:9, color:'#3a3e4a' }}>Updated {LAST_UPDATED}</span>
          </div>
        </header>

        {/* Tabs */}
        <div style={{ borderBottom:'1px solid #1e2129', background:'rgba(10,11,13,0.9)', padding:'0 24px', display:'flex', alignItems:'center', overflowX:'auto', position:'sticky', top:52, zIndex:40 }}>
        ...
        <div style={{ padding:'20px 24px', maxWidth:1400, margin:'0 auto' }}>
            ...
            <div style={{ background:'#13161e', border:'1px solid #1e2129', borderRadius:10, padding:'18px 20px', display:'flex', flexDirection:'column', height:540 }}>
              ...
              <div style={{ overflowY:'auto', flex:1 }}>
                ...
                {topCreators.map((c, i) => {
                  ...
                  return (
                    <div key={c.creator} style={{ padding:'8px 4px', borderBottom:'1px solid rgba(30,33,41,0.5)' }}>
            ...
            <div style={{ background:'#13161e', border:'1px solid #1e2129', borderRadius:10, padding:'18px 20px', display:'flex', flexDirection:'column', height:540 }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12, flexShrink:0 }}>
                ...
                <div style={{ display:'flex', gap:6, alignItems:'center' }}>
                  <input placeholder="creator..." value={videoSearch} onChange={e=>setVideoSearch(e.target.value)}
                    style={{ background:'#1e2129', border:'1px solid #2a2e3a', borderRadius:6, padding:'4px 10px', fontSize:10, fontFamily:"'DM Mono',monospace", color:'#f0f2f8', outline:'none', width:100 }}/>
                      ...
                      <tr key={v.video_id} className="row-hover" style={{ borderBottom:'1px solid rgba(30,33,41,0.4)' }}>
                        ...
                        <td style={{ padding:'8px 6px' }}>
                          <a href={`https://www.tiktok.com/@${v.creator}/video/${v.video_id}`} target="_blank" rel="noopener noreferrer" className="vid-link"
                            style={{ fontFamily:"'DM Mono',monospace", fontSize:9, color:'#57c4ff', display:'flex', alignItems:'center', gap:4, transition:'color 0.15s' }}>
          ...
          <div style={{ marginTop:20, textAlign:'center', fontSize:9, fontFamily:"'DM Mono',monospace", color:'#1e2129', letterSpacing:'0.08em' }}>
            RUFF LINERS · EUKA · TIKTOK SHOP AFFILIATE · DATA AS OF {LAST_UPDATED.toUpperCase()}
          </div>
        </div>
      </div>
    </div>
